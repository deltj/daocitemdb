...
#-------------------------------------------------------------------------------
#
# generate an XML document that can be read by Loki
#
#-------------------------------------------------------------------------------
sub getLokiXml
{
	...
	$writer->comment("XML Generated by daocitemdb");

	$writer->startTag("SCItem");

	$writer->dataElement(ActiveState => "Drop");
	$writer->dataElement(Location => $self->getSlot());
	$writer->dataElement(Realm => $self->getRealm());
	$writer->dataElement(ItemName => $self->getName());
#	$writer->dataElement(AFDPS => "0.0");
#	$writer->dataElement(Bonus => "0");
#	$writer->dataElement(ItemQuality => "0");
#	$writer->dataElement(Equipped => "0");
	$writer->dataElement(Level => $self->getLevel());
#	$writer->dataElement(OFFHAND => "no");
#	$writer->dataElement(SOURCE => "drop");
#	$writer->dataElement(DBSOURCE => "LOKI");
#	$writer->dataElement(CLASSRESTRICTIONS => );
#	$writer->dataElement(ISUNIQUE => );
#	$writer->dataElement(ORACLE_IGNORE => );
#	$writer->dataElement(USER_VALUE => );
#	$writer->dataElement(VARIANT => );
#	$writer->dataElement(ASSOCIATE => );
#	$writer->dataElement(EQUIPLIST => );

	# this is the start of the bonuses
	$writer->startTag("DROPITEM");

	$writer->startTag("SLOT", Number => "0");

#	$writer->dataElement(Remakes => "0");
#	$writer->dataElement(Effect => $);
#	$writer->dataElement(Level => $self->getLevel());
